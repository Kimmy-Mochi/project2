...
$connect = new mysqli('localhost', 'root', 'abc123', 'project2');
    ...
    WHERE teachers.TuserProID = '$ID' ")or die(mysqli_error($connect));
                    ...
                    <table class="table table-striped" id="example" align="center">
                        ...
                        <tbody align="left">
                            <tr>
                                ...
                                $query = mysqli_query($conn->connect(), "SELECT * FROM  `userprofile` WHERE `userProID`='$row[SuserProID]'") or die(mysqli_error($conn->connect()));
                                $fetch = mysqli_fetch_array($query);
                                $query1 = mysqli_query($conn->connect(), "SELECT * FROM  userprofile   INNER JOIN teachers  ON userprofile.userProID = teachers.TuserProID 
                                ...
                                WHERE students.SuserProID ='$row[SuserProID]'") or die(mysqli_error($conn->connect()));
                                $fetch1 = mysqli_fetch_array($query1);

                                $query2 = mysqli_query($conn->connect(), "SELECT * FROM studentappealfiles INNER JOIN studentappeal  ON studentappeal.sAppealID = studentappealfiles.sAppealID 
                                 WHERE  studentappealfiles.sAppealID  = '$row[sAppealID]' ") or die(mysqli_error($conn->connect()));



                                $query3 = mysqli_query($conn->connect(), "SELECT * FROM studentappealfiles INNER JOIN studentappeal  ON studentappeal.sAppealID = studentappealfiles.sAppealID 
                                 WHERE  studentappealfiles.sAppealID  = '$row[sAppealID]' ") or die(mysqli_error($conn->connect()));
