...
$ssid = $_REQUEST['id'];
 ...
 {  
    ...
    //   $d_e= $d_e." ".'23.59.59';//กำหนดเวลาสิ้นสุด
      ...
      $sql = "SELECT  studentID, firstNameTH, lastNameTH,details,datetime  FROM studentappeal inner join userprofile on userprofile.userProID = studentappeal.studentID 
      INNER JOIN advisory on advisory.SuserProID = userprofile.userProID where advisoryStatus = 'M'";   
      $result = mysqli_query($connect->connect(), $sql);
    //   $userPro = $result['studentID'];
      ...
      {       
        $output .= '<tr>  
                        ...
                        <td>'.$row["datetime"].'</td> 
                        <td><form method="post" action="reportAppealingStudent.php?id='.$row["studentID"].'">  
 ...
 {  
      ...
      $obj_pdf->SetTitle("ReportStudentAppeal");  
      ...
      require_once './connectDB.php';
      $connect = new connectDB();
      $sql = "SELECT  studentID,prenameTH, firstNameTH, lastNameTH,details,datetime  FROM studentappeal inner join userprofile on userprofile.userProID = studentappeal.studentID 
      INNER JOIN advisory on advisory.SuserProID = userprofile.userProID 
      INNER JOIN prename on prename.prenameID=userprofile.prenameID where studentID = $ssid and advisoryStatus = 'M'";  
      $result = mysqli_query($connect->connect(), $sql);
      $row = mysqli_fetch_array($result);
      $prename=$row['prenameTH'];

      $sql2="SELECT * from userprofile inner join students on userprofile.userProID = students.SuserProID inner join researchgroup on researchgroup.researchgrID = students.researchGroup 
      ...
      inner join academicplan on academicplan.academicPlanID = students.academicPlanID where userProID = $ssid ";
      $result2 = mysqli_query($connect->connect(), $sql2);
      $row2 = mysqli_fetch_array($result2);
   
     

      $sql1 ="SELECT SuserProID,TuserProID, firstNameTH, lastNameTH FROM advisory INNER JOIN userprofile on advisory.TuserProID = userprofile.userProID where SuserProID = $ssid and advisoryStatus = 'M'";
      $result1 = mysqli_query($connect->connect(), $sql1);
     $row1 = mysqli_fetch_array($result1);
      $content = '';  
      $content .= '  
      <h1 align="center">รายงานข้อมูลของนักศึกษาที่ทำการร้องเรียนอาจารย์ที่ปรึกษา</h1><br />
      <hr noshade width=50% color=black><br><br><br> 
      <table border="0" cellspacing="0" cellpadding="5" align="left">  
     
            <tr>  
                <th width="25%" align=left><b>รหัสนักศึกษาที่ร้องเรียน :</b></th>
                <th width="29%">'.$row["studentID"].'</th> 
                <th width="7%" align=right><b>ชื่อ : </b></th> 
                <th>'.$prename. ''.''.$row["firstNameTH"]. ' '.' '.$row["lastNameTH"].'</th> 
            </tr> 
            <tr>  
                <th width="17%" align=left><b>ระดับการศึกษา :</b></th>
                <th width="27%">'.$row2["educationLevel"].'</th> 
                <th width="17%" align=right><b>แผนการศึกษา :</b></th>
                <th>'.$row2["academicPlan"].'</th> 
              
            </tr> 
            <tr>
                <th width="12%" align=left><b>หลักสูตร : </b></th> 
                <th  width="31%"> '.$row2["curricularTH"].'</th> 
                <th width="18%" align=right><b>กลุ่มวิจัยที่สังกัด : </b></th> 
                <th>'.$row2["researchgrAbbrName"].'</th> 
            </tr>
            <tr>
                <th width="25%" align=left><b>ชื่ออาจารย์ที่ปรึกษาหลัก : </b></th> 
                <th>'.$row1["firstNameTH"]. ' '.' ' .$row1["lastNameTH"].'</th>
            </tr>
            <tr>
                <th width="16%" align=left><b>วันที่ร้องเรียน : </b></th> 
                <th>'.$row["datetime"]. '</th>
            </tr>
            <tr>
                <th width="30%" align=left><b>รายละเอียดในการร้องเรียน</b></th> 
               
            </tr>
            <tr>
                <th width="100%" > &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$row["details"].'</th>
            </tr>
      ';  
    //   $content .= data();  
      $content .= '</table>';  
...
<div class="profile-images-card1">
    ...
    <div class="container">
        <h1 for="" align="center" class="font">รายชื่อข้อมูลของนักศึกษาที่ทำการร้องเรียนอาจารย์ที่ปรึกษา</h1><br>
        <!-- <div class="row">
            <div class="col-md-12">
    ...
    <div class="container">
        ...
        <tr>
            ...
            <!-- <th align="center" class="font">อีเมลล์</th> -->
            <th align="center" class="font">วันที่ร้องเรียน</th>
