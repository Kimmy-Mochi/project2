...
$ssid = $_REQUEST['id'];
 ...
 {  
      ...
      $sql = "SELECT teacherID ,firstNameTH, lastNameTH,detail,datetime  FROM teacherappeal inner join userprofile on userprofile.userProID = teacherappeal.teacherID 
      INNER JOIN advisory on advisory.TuserProID = userprofile.userProID ";   
      ...
      {       
        $output .= '<tr>  
                        ...
                        <td>'.$row["datetime"].'</td> 
                        <td><form method="post" action="reportAppealingTeacher.php?id='.$row["teacherID"].'">  
 ...
 {  
      ...
      $obj_pdf->SetTitle("ReportTeacherAppeal");  
      ...
      $connect = new connectDB();
      $sql = "SELECT  teacherID, firstNameTH, lastNameTH,detail,datetime  FROM teacherappeal inner join userprofile on userprofile.userProID = teacherappeal.teacherID 
      INNER JOIN advisory on advisory.TuserProID = userprofile.userProID  where teacherID = '$tid' and advisoryStatus='M' or advisoryStatus='J'";  
      $result = mysqli_query($connect->connect(), $sql);
      $row = mysqli_fetch_array($result);

    //   $sql2="SELECT * from userprofile inner join teachers on teachers.TuserProID= userprofile.userProID
    //    inner join researchgroup on researchgroup.researchgrID = teachers.researchGroup where userProID = '$tid' ";
    //   $result2 = mysqli_query($connect->connect(), $sql2);
    //   $row2 = mysqli_fetch_array($result2);
    //   $research = $row2['researchgrAbbrName'];
   
      $sql1 ="SELECT SuserProID,TuserProID, firstNameTH, lastNameTH FROM advisory INNER JOIN userprofile on advisory.SuserProID = userprofile.userProID where TuserProID = '$tid' ";
      $result1 = mysqli_query($connect->connect(), $sql1);
      $row1 = mysqli_fetch_array($result1);
      $content = '';  
      $content .= '  
      <h1 align="center">รายงานข้อมูลของอาจารย์ที่ทำการร้องเรียนนักศึกษา</h1><br />
      <hr noshade width=50% color=black><br><br><br> 
      <table border="0" cellspacing="0" cellpadding="5" align="left">  
            <tr>  
                <th width="22%" align=left><b>ชื่ออาจารย์ที่ร้องเรียน :</b></th>
                <th>'.$row["firstNameTH"]. ' '.' '.$row["lastNameTH"].'</th> 
            </tr> 
             <tr>
                <th width="23%" align=left><b>ร้องเรียนนักศึกษา ชื่อ : </b></th> 
                <th width="23%">'.$row1["firstNameTH"]. ' '.' ' .$row1["lastNameTH"].'</th>
                <th width="16%" align=left><b>รหัสนักศึกษา : </b></th> 
                <th>'.$row1["SuserProID"].'</th>
            </tr>
            <tr>
...
<div class="profile-images-card1">
    ...
    <div class="container">
        <h1 for="" align="center" class="font">รายชื่อข้อมูลของอาจารย์ที่ทำการร้องเรียนนักศึกษา</h1><br>
    </div>
